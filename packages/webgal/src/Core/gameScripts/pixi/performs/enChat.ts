...
/**
 * chat 表情：单图层，固定位置出现 1.5s，无移动，结束后自动卸载。
 *
 * 资源：将图片放到 `packages/webgal/public/game/tex/en_chat.png`
 * 调用：pixiInit; pixiPerform:enChat;
 */
const pixiEnChat = (
  tickerKey: string,
  ...
  const texture = PIXI.Texture.from('./game/tex/en_chat.png');
...
registerPerform('enChat', {
  fg: () => {
    ...
    return pixiEnChat('en-chat-foreground-ticker', 'foreground', startX, startY, durationSec, scale);
  },
});
