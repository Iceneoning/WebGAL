...
/**
 * warning 表情：单图层，固定位置出现 1.5s，无移动，结束后自动卸载。
 *
 * 资源：将图片放到 `packages/webgal/public/game/tex/en_warning.png`
 * 调用：pixiInit; pixiPerform:enWarning;
 */
const pixiEnWarning = (
  tickerKey: string,
  ...
  const texture = PIXI.Texture.from('./game/tex/en_warning.png');
...
registerPerform('enWarning', {
  fg: () => {
    ...
    return pixiEnWarning('en-warning-foreground-ticker', 'foreground', startX, startY, durationSec, scale);
  },
});
