...
/**
 * doubt 表情：单图层，固定位置出现 1.5s，无移动，结束后自动卸载。
 *
 * 资源：将图片放到 `packages/webgal/public/game/tex/en_doubt.png`
 * 调用：pixiInit; pixiPerform:enDoubt;
 */
const pixiEnDoubt = (
  tickerKey: string,
  ...
  const texture = PIXI.Texture.from('./game/tex/en_doubt.png');
...
registerPerform('enDoubt', {
  fg: () => {
    ...
    return pixiEnDoubt('en-doubt-foreground-ticker', 'foreground', startX, startY, durationSec, scale);
  },
});
