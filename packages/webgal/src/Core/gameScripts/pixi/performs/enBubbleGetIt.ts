...
/**
 * getit 气泡表情：双图层（Bubble + 中间图片），固定位置出现 1.5s，无移动，末尾淡出后自动卸载。
 *
 * 资源：
 * - 气泡：`packages/webgal/public/game/tex/Bubble.png`
 * - 中间图：`packages/webgal/public/game/tex/en_getit.png`
 *
 * 调用：pixiInit; pixiPerform:enBubbleGetIt;
 */
const pixiEnBubbleGetIt = (
  tickerKey: string,
  ...
  // 上层：中间图片（getit）
  const centerTexture = PIXI.Texture.from('./game/tex/en_getit.png');
...
registerPerform('enBubbleGetIt', {
  fg: () => {
    ...
    return pixiEnBubbleGetIt(
      'en-bubble-getit-foreground-ticker',
      'foreground',
      startX,
      startY,
      durationSec,
