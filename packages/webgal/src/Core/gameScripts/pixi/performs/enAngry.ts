...
/**
 * angry 表情：单图层，固定位置出现 1.5s，无移动，结束后自动卸载。
 *
 * 资源：将图片放到 `packages/webgal/public/game/tex/en_angry.png`
 * 调用：pixiInit; pixiPerform:enAngry;
 */
const pixiEnAngry = (
  tickerKey: string,
  ...
  const texture = PIXI.Texture.from('./game/tex/en_angry.png');
...
registerPerform('enAngry', {
  fg: () => {
    ...
    return pixiEnAngry('en-angry-foreground-ticker', 'foreground', startX, startY, durationSec, scale);
  },
});
