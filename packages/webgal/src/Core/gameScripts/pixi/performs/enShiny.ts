...
/**
 * shiny 表情：单图层，固定位置出现 1.5s，无移动，结束后自动卸载。
 *
 * 资源：将图片放到 `packages/webgal/public/game/tex/en_shiny.png`
 * 调用：pixiInit; pixiPerform:enShiny;
 */
const pixiEnShiny = (
  tickerKey: string,
  ...
  const texture = PIXI.Texture.from('./game/tex/en_shiny.png');
...
registerPerform('enShiny', {
  fg: () => {
    ...
    return pixiEnShiny('en-shiny-foreground-ticker', 'foreground', startX, startY, durationSec, scale);
  },
});
