...
/**
 * upset 气泡表情：双图层（Bubble + 中间图片），固定位置出现 1.5s，无移动，末尾淡出后自动卸载。
 *
 * 资源：
 * - 气泡：`packages/webgal/public/game/tex/Bubble.png`
 * - 中间图：`packages/webgal/public/game/tex/en_upset.png`
 *
 * 调用：pixiInit; pixiPerform:enBubbleUpset;
 */
const pixiEnBubbleUpset = (
  tickerKey: string,
  ...
  // 上层：中间图片（upset）
  const centerTexture = PIXI.Texture.from('./game/tex/en_upset.png');
...
registerPerform('enBubbleUpset', {
  fg: () => {
    ...
    return pixiEnBubbleUpset(
      'en-bubble-upset-foreground-ticker',
      'foreground',
      startX,
      startY,
      durationSec,
