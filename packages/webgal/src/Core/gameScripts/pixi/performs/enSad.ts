...
/**
 * sad 表情：单图层，固定位置出现 1.5s（可选轻微下滑），结束后自动卸载。
 *
 * 资源：将图片放到 `packages/webgal/public/game/tex/en_sad.png`
 * 调用：pixiInit; pixiPerform:enSad;
 */
const pixiEnSad = (
  tickerKey: string,
  ...
  const texture = PIXI.Texture.from('./game/tex/en_sad.png');
  ...
  const tickerFn = (delta: number) => {
    ...
    // 可选轻微向下滑动：不想动就把 moveDownPx 设为 0
    sprite.y = startY + moveDownPx * e;
...
registerPerform('enSad', {
  fg: () => {
    ...
    // 轻微向下滑动（像素）：完全不移动就改成 0
    const moveDownPx = 5;

    return pixiEnSad('en-sad-foreground-ticker', 'foreground', startX, startY, durationSec, scale, moveDownPx);
  },
});
